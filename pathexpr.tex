...
\begin{proof}
  ...
  \zak{I would not expect $S_{v+1} = S_v \cup S_3$ -- I don't know if
    ...
    $\mathit{eval}_{v+1}(u) = (\mathit{noncycle} \cdot \mathtit{cycle}^*) \cdot (q \cdot p) = (\mathit{noncycle} \cdot \mathtit{cycle}^*) \cdot \mathit{eval}_v(u)$.  But if $u$ is not a predecessor of $v$, then we don't compress, and
    $\mathit{eval}_{v+1}(u) = ((\mathit{noncycle} \cdot \mathtit{cycle}^*) \cdot q) \cdot p$.
    Maybe you could instead define $\mathit{Links}_v = \bigcup_{u < v} (\mathit{noncycle}_u \cdot \mathtit{cycle}_u^*)$ and $\mathit{LinkSeq}_v$ to be the set of all path expressions that can be formed by concatenating path expressions from $\mathtit{Links}_v$ (finitely many, due to type constraints).  Then prove the result about sub-expreesions of path expressions from $\mathtit{LinkSeq}_v$.
  }
...
\zak{init/fin should be src/tgt?  Commas are ambigious -- should $w \dominates init(P_1), fin(P_1)$ be interpreted as $w$ dominates both $\mathit{src}(P_1)$ and $\mathtit{tgt}(P_1$)?  (Expanding to English is easier on the reader than the compact mathematical notation)}

  Similarly, take any $P_2 \in S_2$. By Lemma
  \ref{tarjan:correct}, $v \dominates init(P_2), fin(P_2)$. Let
  $L = L(noncycle \cdot cycle^*)$ \zak{$L$ is already reserved for the length function -- pick a different symbol}. Then,
